"""Miscellaneous, non-numerical utilities (logging, descriptive summaries)."""
import os
import logging
...
def proc_freq(df: pd.DataFrame, columns: list):
    """Logs the frequency, percentage, and cumulative percentage of each specified column or tuple of columns in the DataFrame.
    ...
    Note: output is emitted via the module logger (INFO level) rather than
    printed directly; configure a handler (e.g. via `setup_logger`) to see it.
    """
