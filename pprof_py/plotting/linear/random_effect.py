"""Plotting for `LinearRandomEffectModel`: funnel plots, provider-effect
...
class _LinearREPlottingHost(Protocol):
    """Attribute contract that `RandomEffectPlottingMixin` expects from its
...
class RandomEffectPlottingMixin:
    ...
    def plot_coefficient_forest(
        ...
        beta = self.coefficients_["beta"].values.flatten()
        se_beta = np.sqrt(np.diag(self.variances_["beta"]))
        df_denom = self.fitted_.size - len(beta) - len(self.coefficients_["alpha"])
        ...
        coef_df = (
            pd.DataFrame({
                "covariate": self.coefficients_["beta"].index,
                "estimate": beta,
                "ci_lower": lower,
                "ci_upper": upper
            })
    ...
    def plot_residuals(
        ...
        ax.scatter(
            np.asarray(self.fitted_), np.asarray(self.residuals_),
    ...
    def plot_qq(
        ...
        residuals_flat = np.asarray(self.residuals_).ravel()
