...
def plot_caterpillar(
    ...
    point_color_default: str = _style.COLOR_PRIMARY, # Default if no flags or flag not in dict
    flag_colors: Dict[int, str] = _style.FLAG_COLORS,
    point_alpha: float = _style.POINT_ALPHA,
    line_size: float = _style.LINE_WIDTH, # For refline
    refline_value: Optional[float] = 0.0, # Default refline at 0
    refline_linestyle: str = _style.REFLINE_LINESTYLE, # Changed name for clarity
    refline_color: str = _style.COLOR_REFERENCE, # Changed default
    font_size: float = _style.FONT_SIZE,
    tick_label_size: float = _style.TICK_LABEL_SIZE,
    errorbar_size: float = _style.ERRORBAR_SIZE,
    errorbar_alpha: float = _style.ERRORBAR_ALPHA, # Matched funnel fill alpha
    # Error bar color will match point color if flags are used
    errorbar_color_default: str = _style.COLOR_NEUTRAL_GREY, # Default if no flags or flag not in dict
