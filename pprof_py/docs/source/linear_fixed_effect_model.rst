...
The implementation (``LinearFixedEffectModel.__init__``) provides two options via the ``gamma_var_option`` parameter for estimating the diagonal elements of :math:`\text{Var}(\hat{\boldsymbol\gamma})`:
...
Let :math:`\hat{\boldsymbol{\beta}}` and :math:`\hat{\boldsymbol{\gamma}}` be the OLS estimates. Define a reference or baseline provider effect :math:`\gamma_0` (e.g., median or mean of :math:`\hat{\gamma}_i`, as specified by the ``null`` parameter in ``LinearFixedEffectModel.calculate_standardized_measures``).
...
.. Therefore, for linear fixed effect models, both indirect and direct standardized *differences* are equivalent to :math:`\hat{\gamma}_i - \gamma_0`. The implementation (``LinearFixedEffectModel.calculate_standardized_measures``) calculates and returns this difference along with the observed sum (:math:`n_i \bar{Y}_i`) and expected sum (:math:`n_i \bar{E}_i(\gamma_0)` for indirect, or :math:`N \bar{E}^{(0)}` and :math:`N \bar{E}^{(k)}` components for direct) components.

2.4. Hypothesis Testing for Provider Effects
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

We test the null hypothesis :math:`H_0: \gamma_i = \gamma_0` against an alternative :math:`H_1`. Given the linear model assumptions and the estimation of :math:`\sigma^2`, the natural test is a **t-test**, as implemented in ``LinearFixedEffectModel.test``:
...
*   **For** :math:`\beta_k` (see ``LinearFixedEffectModel.summary``):
...
*   **For** :math:`\gamma_i` (see ``LinearFixedEffectModel.calculate_confidence_intervals``):
