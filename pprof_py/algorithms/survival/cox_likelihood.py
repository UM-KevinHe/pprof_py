...
user-facing API -- it is pure NumPy in, NumPy out, which is exactly the
seam a future Spark/distributed backend would replace (each partition
computing its own per-stratum StratumContribution and summing across
partitions, unchanged from how strata are already summed here). See
